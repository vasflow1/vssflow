...
class ESPnetMultiTaskDataset(AbsDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.iterator_map = {}
        self.datasets = []
        for triplet in path_name_type_list:  # 不同的数据集json文件，对于每一个数据集，做下面的处理
            ...
            for triplet in json_dict["data_files"]:  # 每个数据集json文件中都存在一个data_files项，即text，utt2spk这些
            ...
            example_list = json_dict["data_files"][0].strip().split(",")[0]  # 对每个数据集，读入basename
            if _type == "jsonl":
                example_list = [
            ...
            dataset = EspnetSpeechLMDataset(
                ...
                task=json_dict["task"],
                structured_items=["conti_feats", "token_dict"],
                **kwargs,
            )  # 把相应的数据传入
            self.datasets.append(dataset)  # 把建立好的数据集加入到列表中
