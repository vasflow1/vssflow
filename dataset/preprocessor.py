            ...
            if modality in ["text_bpe", "g2p"]:
                if isinstance(value, str):
                    ...
                    if modality == "g2p":
                        ...
                        value = ["g2p_'" if tok==',' else f"g2p_{tok}" for tok in value]
...
class VSpeechLMPreprocessor(AbsPreprocessor):
    ...
    def __call__(
        ...
        for idx, triplet in enumerate(task.data_triplets):
            ...
            if name == task.fixed_length_key:
                inference_length = value.shape[0] / self.codec_token_in_use
                # logging.info(f"fixed_length_key {inference_length}")

            # logging.info(f"inter_segment_pad {self.inter_segment_pad}")
            if idx != len(task.data_triplets) - 1  and self.inter_segment_pad > 0:
                pad = np.tile(self.special_token("<pad>"), self.inter_segment_pad)
                value = np.concatenate([value, pad], axis=0)
            cache[(name, modality)] = value
            # logging.info(f"2 every value {name} {value.shape}")
            seqs.append(value)
        ...
        if self.encoder_decoder_format:
            ...
            # logging.info(f'3 dec_seq_shape {new_data["dec_seq"].shape}')
        
        if np.isin(self.unk, new_data["dec_seq"]):
            raise ValueError(f"Unknown token is in the decoder seq. UID: {uid}")
        prefix_len = sum([len(seq) for seq in seqs[:n_conditions]])
        prefix_len = prefix_len // self.codec_token_in_use + 2
        # logging.info(f"4 prefix_len {prefix_len}")
        new_data["prefix_len"] = np.array([prefix_len])
        ...
        for idx, conti_feat in enumerate(conti_feats, 1):
            ...
            bias = sum(len(seg) for seg in prev_segs) // self.codec_token_in_use
            conti_emb, start, end = conti_feat
            # logging.info(f"5 conti_bias {start + bias} {end + bias}")
            new_conti_feats.append((conti_emb, start + bias, end + bias))
        new_data["conti_feats"] = new_conti_feats
    ...
    def modality_specific_processing(self, value, modality, cache, uid):
        ...
        if modality in ["codec", "spk", "codec_ssl"]:
            ...
            if modality == "spk":
                ...
                if "codec_ssl" in all_modalities:
                    ...
                    value = value + self.token_bias["codec"][0]
                    # logging.info(f"spk {value.shape}")
        ...
        # continuous modalities
            ...
            # is no overlap between tokens and continuous embeddings when using
            # delay interleave.
            value_len = conti_emb.shape[0] + self.codec_token_in_use - 1  # 但是这里value_len其实是不等于conti长度的，它赠长了一部分
            value = np.tile(value, value_len)  # 但是指示的长度还在原来的位置，也就是说，value其实开始的更早，只是前面被填充了一些内容

            # embedidngs, start, end
            conti_feat = (  # conti_emb就是原来的长度，指示的开始和结束符之差也和conti的长度相等
                conti_emb,
                self.codec_token_in_use,
                self.codec_token_in_use + conti_emb.shape[0],
            )
            # logging.info(f"conti_feat {conti_emb.shape} {value.shape} {self.codec_token_in_use} {self.codec_token_in_use + conti_emb.shape[0]}")
...
class VSpeechLMTVAttnPreprocessor(AbsPreprocessor):
    """Preprocessor specifically for SpeechLM models"""
    ...
    def __init__(
        ...
        self.pad = token_list.index("<pad>")
        self.unk = token_list.index("<unk>")
        self.additional_seq_list = additional_seq_list
    ...
    def __call__(
        ...
        # 这里应该额外返回text和video序列，让它们能够在collate函数中被pad到相同的长度
        # 这样就可以直接在模型中做attention
        # 除此之外，还要返回video部分的开始和结束位置（不用真的写，在conti里已经有了），
        # 从而能将attention之后的结果直接加到video那部分中去
        new_data = {}
        ...
        for idx, triplet in enumerate(task.data_triplets):
            ...
            value, conti_feat = self.modality_specific_processing(data[name], modality, cache, uid)
            
            if name == task.fixed_length_key:  # 不会进入
                inference_length = value.shape[0] / self.codec_token_in_use

            if idx != len(task.data_triplets) - 1  and self.inter_segment_pad > 0:  # 不会进入
                pad = np.tile(self.special_token("<pad>"), self.inter_segment_pad)
                value = np.concatenate([value, pad], axis=0)
                logging.info(f"self.inter_segment_pad: {self.inter_segment_pad}")
            
            # if name in list: new_data[name_seq] = value  这里的list可以在build_iter_option中定义
            if name in self.additional_seq_list:  # here
                new_data[f"{name.split('.')[0]}_seq"] = value.reshape(-1, self.codec_token_in_use)[1:]
                if conti_feat is not None:
                    new_data[f"{name.split('.')[0]}_seq"] = conti_feat[0]
    ...
    def modality_specific_processing(self, value, modality, cache, uid):
        ...
        # continuous modalities
            ...
            # is no overlap between tokens and continuous embeddings when using
            # delay interleave.
            value_len = conti_emb.shape[0] + self.codec_token_in_use - 1  # 但是这里value_len其实是不等于conti长度的，它增长了一部分
            value = np.tile(value, value_len)  # 但是指示的长度还在原来的位置，也就是说，value其实开始的更早，只是前面被填充了一些内容
            ...
            # timestamps = (self.codec_token_in_use, self.codec_token_in_use + conti_emb.shape[0])  # 这里的问题在于会不会把模态符算进去
        else:
            raise NotImplementedError
...
class VSpeechLMRepeatPreprocessor(AbsPreprocessor):
    """Preprocessor specifically for SpeechLM models"""
    ...
    def __call__(
        ...
        for idx, triplet in enumerate(task.data_triplets):
            ...
            # if name in list: new_data[name_seq] = value  这里的list可以在build_iter_option中定义
            # PS: 这里的两个seq是用来做attention的，因此这里不用把text pad，因为它做完attention之后就不会再用到了
            if name in self.additional_seq_list:  # here
                new_data[f"{name.split('.')[0]}_seq"] = value.reshape(-1, self.codec_token_in_use)[1:]
                if conti_feat is not None:
                    new_data[f"{name.split('.')[0]}_seq"] = conti_feat[0]
        ...
        # 在这里应该加一个操作，把text feature pad到video feature相同的长度
        text_seq = seqs[text_idx]
        # video_seq = seqs[video_idx]
        # 应该在text_seq后面pad 0
        # pad_len = video_seq.shape[0] - text_seq.shape[0]
        pad_len = new_data["video_seq"].shape[0] * 8 - text_seq.shape[0] + 8
        if pad_len < 0:
            logging.info(f"0000000000000000 {uid}")
            # print(new_data["video_seq"].shape[0])
            text_seq = text_seq[:text_seq.shape[0]+pad_len]
        else:
            text_seq = np.pad(text_seq, (0, pad_len), mode="constant", constant_values=self.pad)
        seqs[text_idx] = text_seq
        ...
        if self.encoder_decoder_format:
            ...
            new_data["dec_seq"] = np.concatenate(
                [sos_eos] + [task_identifier] + seqs[:n_conditions-1] + seqs[n_conditions:] + [sos_eos], axis=0
            ).reshape(-1, self.codec_token_in_use)[: self.n_ctx]
        
        if np.isin(self.unk, new_data["dec_seq"]):
            raise ValueError(f"Unknown token is in the decoder seq. UID: {uid}")
        prefix_len = sum([len(seq) for seq in seqs[:n_conditions-1]])  # 不要video 
        prefix_len = prefix_len // self.codec_token_in_use + 2
        new_data["prefix_len"] = np.array([prefix_len])
...
class VSpeechLMRepeatDurationPreprocessor(AbsPreprocessor):
    """Preprocessor specifically for SpeechLM models"""
    ...
    def __call__(
        ...
        for idx, triplet in enumerate(task.data_triplets):
            ...
            if name in self.additional_seq_list:  # here
                if value is not None:
                    new_data[f"{name.split('.')[0]}"] = value.reshape(-1, self.codec_token_in_use)[1:]
                else:
                    new_data[f"{name.split('.')[0]}"] = conti_feat
        ...
        # 在这里应该加一个操作，把text feature pad到[video_lenth]相同的长度 因为duration sum是gt
        text_seq = seqs[text_idx]
        pad_len = new_data["video"].shape[0] * 2  # video帧扩展2倍 近似等于duration的长度
        pad_len = (pad_len + 1) * 8 - text_seq.shape[0]  # 理想长度是duration的长度+modality标识符 -本来就有的长度
        if pad_len < 0:  # 判断pad_len的合法性
            logging.info(f"0000000000000000 {uid}")
            print(new_data["video"].shape[0])
            text_seq = text_seq[:text_seq.shape[0]+pad_len]
        else:
            text_seq = np.pad(text_seq, (0, pad_len), mode="constant", constant_values=self.pad)
        # logging.info(f"pad text seq length: {text_seq.shape}")
        seqs[text_idx] = text_seq
        cache[("phoneme", "g2p")] = text_seq
        ...
        else:  ### 构造seq，不要video和duration phoneme+utt2spk+wav
            new_data["dec_seq"] = np.concatenate(
                [sos_eos] + [task_identifier] + seqs[:n_conditions-2] + seqs[n_conditions:] + [sos_eos], axis=0
            ).reshape(-1, self.codec_token_in_use)[: self.n_ctx]
        
        if np.isin(self.unk, new_data["dec_seq"]):
            raise ValueError(f"Unknown token is in the decoder seq. UID: {uid}")
        prefix_len = sum([len(seq) for seq in seqs[:n_conditions-2]])  # 不要video和duration
        prefix_len = prefix_len // self.codec_token_in_use + 2
        new_data["prefix_len"] = np.array([prefix_len])

        # logging.info(f"dec_seq: {new_data['dec_seq'].shape}")
        # logging.info(f"prefix_len: {new_data['prefix_len']}")
        ...
        #     new_conti_feats.append((conti_emb, start + bias, end + bias))
        # new_data["conti_feats"] = new_conti_feats
        new_data["conti_feats"] = None  # 临时改成这个，因为这个setting里面没有要用的conti_feat
    ...
    def modality_specific_processing(self, value, modality, cache, uid):
        ...
        # Other discrete modalities
            ...
            if modality in ["text_bpe", "g2p"]:  # 这里的g2p模态是phoneme，所以不需要额外的处理
                if isinstance(value, str):
                    # try:
                    #     value = self.text_cleaner(value)
                    # except:
                    ...
                    if modality == "g2p":
                        # value = [f"g2p_{tok}" for tok in value]
                        value = [tok if tok=='<blank>' else f"g2p_{tok}" for tok in value]
                    value = self.converter.tokens2ids(value)
                    value = np.array(value)
        ...
        elif modality in ["mfa"]:  # 为了duration新添加的modality
            # 构造groundtruth score
            # return None, value [2,3,4]
            if modality in ["mfa"]:
                # phoneme_length = len(duration)  wav_length = duration.sum()
                gt_score = np.zeros([value.shape[0], value.sum()])
                # logging.info(f"gt_score {gt_score.shape}")
                d = 0
                for i in range(value.shape[0]):
                    gt_score[i, d:d+value[i]] = 1
                    d += value[i]

                gt_score[value.shape[0] - 1, value.sum():] = 1

                gt_score_path = f"/nfs-03/yuyue/DSU-AVO-main/output_aligner/DSU_LRS2_for_espnet/result/100k/attn/{uid.split(' ')[-1]}_attn_aligner.npy"
                # gt_score_path = f"/nfs-03/yuyue/DSU-AVO-main/output_aligner/Chem_06/result/2k/attn/{uid.split(' ')[-1].replace('chem_00_', '')}_attn_aligner.npy"
                gt_score = np.load(gt_score_path)
                return None, gt_score.transpose(1, 0)  # duration不会被编入seq中，不参与SpeechLM的input，因此也不不需要有modality标识符
            

        # continuous modalities
        elif modality in ["text_emb", "video_feat"]:
            if modality == "video_feat":
                return None, value
            else:
                if value.ndim != 2:
                    raise ValueError(f"Text embedding should have size of [T, D]")
...
class VSpeechLMDurationPredictorPreprocessor(AbsPreprocessor):
    """Preprocessor specifically for SpeechLM models"""
        ...
        for idx, triplet in enumerate(task.data_triplets):
            ...
            if name in self.additional_seq_list:  # here
                if name == "phoneme":
                    ...
                    new_data[f"{name.split('.')[0]}"] = conti_feat[:new_data["video"].shape[0] * 2, :] # gt attn
                    if conti_feat.shape[0] < new_data["video"].shape[0] * 2:
                        # print(f"confi_feat: {conti_feat.shape}")
                        # print(f"video_len: {new_data['video'].shape[0] * 2}")
                        pad_len = conti_feat.shape[0] - new_data["video"].shape[0] * 2
                        new_data["duration"] = np.vstack([conti_feat, conti_feat[pad_len:, :]])
                        # print(f"after stack: {new_data['duration'].shape}")
...
class VSpeechLMDiscreteDurationPredictorPreprocessor(AbsPreprocessor):
    """Preprocessor specifically for SpeechLM models"""
        ...
        for idx, triplet in enumerate(task.data_triplets):
            ...
            if name in self.additional_seq_list:  # here
                if name == "phoneme":
                    ...
                    new_data[f"{name.split('.')[0]}"] = conti_feat
                elif name =="duration":  # 将gt attn 控制在和video_len相符的维度
                    new_data[f"{name.split('.')[0]}"] = value  # gt duration
                    new_data["gt_attn"] = conti_feat[:new_data["video"].shape[0] * 2, :] # gt attn
    ...
    def modality_specific_processing(self, value, modality, cache, uid):
        ...
        # Other discrete modalities
            ...
            if modality in ["mfa"]:
                ...
                return duration, gt_score.transpose(1, 0)
...
class VSpeechLMAttnExpandPreprocessor(AbsPreprocessor):
    """Preprocessor specifically for SpeechLM models"""
    ...
    def modality_specific_processing(self, value, modality, cache, uid):
        ...
        # Other discrete modalities
            ...
            if modality in ["mfa"]:
                # phoneme_length = len(duration)  wav_length = duration.sum()
                gt_score = np.zeros([value.shape[0], value.sum() + 8])
                # logging.info(f"gt_score {gt_score.shape}")
                d = 0
                for i in range(value.shape[0]):
                    gt_score[i, d:d+value[i]] = 1
                ...
                return None, gt_score  # duration不会被编入seq中，不参与SpeechLM的input，因此也不不需要有modality标识符
...
class VSpeechLMAllAttnPreprocessor(AbsPreprocessor):
    """Preprocessor specifically for SpeechLM models"""
