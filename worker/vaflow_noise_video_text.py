...
class VAFlow(pl.LightningModule):
    def __init__(self, 
        ...
        self.ref_proj = nn.Sequential(
            nn.Linear(256, synch_feat_dim * 2, bias=False),
            nn.SiLU(),
            nn.Linear(synch_feat_dim * 2, synch_feat_dim, bias=False),
        )
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        # crossattn cond
        synch_feature       = torch.concat([ref_speech_cond, synch_feature], dim = 1)                      # [bs, 1+latent_length, synch_feat_dim(768)]
        ref_speech_cond_pad = torch.zeros([batch_size, 1, lip_feature.shape[-1]]).to(device)               # [bs, 1,               lip_feat_dim]
        lip_feature         = torch.concat([ref_speech_cond_pad, lip_feature], dim = 1)                    # [bs, 1+latent_length, lip_feature]
        cross_attn_cond     = torch.concat([synch_feature,  lip_feature], dim = -1)                        # [bs, 1+latent_length, synch_feat_dim + phone_latent_dim]

        # concat cond
        video_feat_cond = video_feat_cond.transpose(1,2)                                # [bs, 768, latent_length]
        phone_latent = phone_latent.transpose(1,2)                                      # [bs, phone_latent_dim, latent_length]
        latent_cond = torch.concat([video_feat_cond, phone_latent], dim=1)              # [bs, 768 + phone_latent_dim (800), latent_length]
    ...
    def validation_step_(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        ref_speech_cond_pad = torch.zeros([batch_size, 1, lip_feature.shape[-1]]).to(device)               # [bs, 1,               lip_feat_dim]
        lip_feature         = torch.concat([ref_speech_cond_pad, lip_feature], dim = 1)                    # [bs, 1+latent_length, lip_feature]
        cross_attn_cond     = torch.concat([synch_feature,  lip_feature], dim = -1)                        # [bs, 1+latent_length, synch_feat_dim + phone_latent_dim (1792)]
        ...
        latent_uncond = None
        latent_cond = torch.concat([video_feat_cond, phone_latent], dim=1)              # [bs, 768 + phone_latent_dim (800), latent_length]
        latent_uncond = torch.zeros_like(latent_cond)                                   # [bs, 768 + phone_latent_dim (800)， latent_length]  
