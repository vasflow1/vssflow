...
class VAFlow(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        # ref_speech_cond = ref_speech_cond.transpose(1,2)                            # [bs, 768, 1]
        ...
        latent_cond   = torch.concat([video_feat_cond, phone_latent, lip_feature, synch_feature], dim = 1)    # [bs, 768 + phone_latent_dim + lip_feat_dim + synch_feat_dim, latent_length]
        ...
        if random.random() < self.unconditional_prob:
            ...
            ref_speech_cond = torch.zeros_like(ref_speech_cond, dtype = ref_speech_cond.dtype).to(device)
    ...
    def validation_step_(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        latent_uncond = None
        latent_cond   = torch.concat([video_feat_cond, phone_latent, lip_feature, synch_feature], dim = 1)    # [bs, 768 + phone_latent_dim + lip_feat_dim + synch_feat_dim, latent_length]
        latent_uncond = torch.zeros_like(latent_cond)                                                         # [bs, 768 + phone_latent_dim + lip_feat_dim + synch_feat_dim, latent_length]
