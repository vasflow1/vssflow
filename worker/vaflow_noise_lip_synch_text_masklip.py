...
class VAFlow(pl.LightningModule):
    def __init__(self, 
                 ...
                 # Mask setting
                 training_mask_ratio      : list = [0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.90,1.00],
                 val_mask_ratio           : list = [0.10,0.20,0.30,0.60,1.00],
                 shuffle_ratio            : int  = 0,
                 # Training setting
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        # MODIFIED: random drop lip and synch
        mask_ratio = random.choice(self.training_mask_ratio)
        rand_mask = torch.rand(batch_size, 1, lip_feature.shape[-1], device=lip_feature.device)
        mask = (rand_mask > mask_ratio).float()
        lip_feature = lip_feature * mask
        rand_mask = torch.rand(batch_size, 1, synch_feature.shape[-1], device=synch_feature.device)
        mask = (rand_mask > mask_ratio).float()
        synch_feature = synch_feature * mask
        # MODIFIED: random flip lip
        if random.random() <= self.shuffle_ratio: 
            idx = torch.randperm(lip_feature.shape[-1]).to(lip_feature.device)
            lip_feature = lip_feature[:, :, idx]
    ...
    def on_validation_epoch_start(self):
        ...
        ## MODIFIED
        for mask_ratio in self.val_mask_ratio:
            os.makedirs(f"{self.val_log_dir_for_audio_per_epoch}/{mask_ratio}", exist_ok=True)
            os.makedirs(f"{self.val_log_dir_for_speech_per_epoch}/{mask_ratio}", exist_ok=True)
        ...
        for mask_ratio in self.val_mask_ratio:
            ...
            for _rand_i in range(self.num_samples_per_prompt):
                ...
                for i, audio in enumerate(gen_audio):
                    ...
                    # MODIFIED
                    audio_path = os.path.join(as_path, str(mask_ratio), "{}.wav".format(video_id[i]))
                    if self.num_samples_per_prompt != 1:
                        audio_path = os.path.join(as_path, str(mask_ratio), "{}_{:02d}.wav".format(video_id[i], _rand_i))
