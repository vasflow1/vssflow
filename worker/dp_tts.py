...
class DurationPredictor(pl.LightningModule):
    ...
    def get_input(self, batch, use_cache_video_feat=False):
        ...
        # Type convert
        duration_span = duration_span.float() if duration_span is not None else duration_span
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        # Loss
        loss = self.loss_function_attn(final_attn_weight, duration_matrix, avhubert_length, phone_length)
    ...
    def validation_step_(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        # [bs]
        # max_speech_pos = torch.tensor([torch.where(t != 0)[0][-1].item() if (t != 0).any() else -1 for t in duration_span.sum(-1)])
    ...
    def predict_step(self, batch, batch_idx=0, dataloader_idx=0,):
        with torch.no_grad():
            #          [bs, 250, max_pho_len]  [bs, max_pho_len, 2]    [bs, 250, 1024]    10*[audio_len]    [bs, max_pho_len]  10*[pho_len]
            _,                              _,              _,          avhubert,         avhubert_length,   phone_id,          phone_length = self.get_input(batch)
            batch_size = len(avhubert_length)
