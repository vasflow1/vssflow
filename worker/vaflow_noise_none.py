...
class VAFlow(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        ref_speech_cond_pad = torch.zeros([batch_size, 1, (phone_latent.shape[-1] + 
                                                            lip_feature.shape[-1] + 
                                                            synch_feature.shape[-1])]).to(device)            # [bs, 1,               phone_latent_dim + synch_feat_dim + lip_feat_dim]
        other_cond          = torch.concat([phone_latent, lip_feature, synch_feature], dim=-1)               # [bs, latent_length，  phone_latent_dim + synch_feat_dim + lip_feat_dim]
        other_cond          = torch.concat([ref_speech_cond_pad, other_cond], dim = 1)                       # [bs, 1+latent_length, phone_latent_dim + synch_feat_dim + lip_feat_dim]
        cross_attn_cond     = torch.concat([video_feat_cond,  other_cond], dim = -1)                         # [bs, 1+latent_length, 768 + phone_latent_dim + synch_feat_dim + lip_feat_dim]
    ...
    def validation_step_(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        ref_speech_cond_pad = torch.zeros([batch_size, 1, (phone_latent.shape[-1] + 
                                                            lip_feature.shape[-1] + 
                                                            synch_feature.shape[-1])]).to(device)            # [bs, 1,               phone_latent_dim + synch_feat_dim + lip_feat_dim (1824)]
        other_cond          = torch.concat([phone_latent, lip_feature, synch_feature], dim=-1)               # [bs, latent_length，  phone_latent_dim + synch_feat_dim + lip_feat_dim]
        other_cond          = torch.concat([ref_speech_cond_pad, other_cond], dim = 1)                       # [bs, 1+latent_length, phone_latent_dim + synch_feat_dim + lip_feat_dim]
        cross_attn_cond     = torch.concat([video_feat_cond,  other_cond], dim = -1)                         # [bs, 1+latent_length, 768 + phone_latent_dim + synch_feat_dim + lip_feat_dim]
        cross_attn_cond_uncond = torch.zeros_like(cross_attn_cond)                                           # [bs, 1+latent_length, 768 + phone_latent_dim + synch_feat_dim + lip_feat_dim]
