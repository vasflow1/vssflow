...
from RawNet.python.RawNet3.utils import tuneThresholdfromScore, ComputeErrorRates, ComputeMinDcf
...
def flac_to_16k_wav_inplace(flac_path):
    ...
    if wav_path.exists():
        try:
            ...
            if info.sample_rate == 16000 and info.num_channels == 1:
                return str(wav_path)
        except:
            pass  # 出错了就重新转
    
    waveform, sr = torchaudio.load(flac_path)
    if waveform.size(0) > 1:  # 转单声道
...
if __name__ == "__main__":
    ...
    args.inference_utterance = True

    audio_dir = '/home/chengxin/chengxin/dataset/V2C/speakers'
    out_dir = '/home/chengxin/chengxin/dataset/V2C/speaker_emb'
    for spk in tqdm(os.listdir(audio_dir)):
        out_dir_spk = f"{out_dir}/{spk}"
        os.makedirs(out_dir_spk, exist_ok=True)
        for file in tqdm(glob(f"{audio_dir}/{spk}/**/*.flac", recursive = True)):
            ### file 读取后保存为16000hz的wav文件
            wav_file = flac_to_16k_wav_inplace(file)          # ← 新增这行
            args.input = wav_file
            args.out_dir = f"{out_dir_spk}/{file.split('/')[-1][:-5]}.npy"
            print(args.input, args.out_dir)
            main(args)
