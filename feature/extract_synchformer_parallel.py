...
def split_into_chunks(items: list[Path], num_chunks: int) -> list[list[Path]]:
    if num_chunks <= 0:
...
def extract(video_paths, gt_cache, audio_length=8.0, num_workers=4, batch_size=8, device_id='cuda:0', synchformer_ckpt=None):
    """
    ...
    # video_paths = [f for f in video_names if f.endswith('.mp4')]
...
# nohup python feature/extract_synchformer_parallel.py \
#   --video_path /home/chengxin/chengxin/dataset/VGGSound/data \
#               /home/chengxin/chengxin/dataset/Chem/sentence_video_25fps \
#               /home/chengxin/chengxin/dataset/GRID/videos_25fps_mp4 \
#               /home/chengxin/chengxin/dataset/LRS2/video_25fps \
#               /home/chengxin/chengxin/dataset/V2C/dataset \
#   --gt_cache /home/chengxin/chengxin/dataset/Dataset_feature/v2c_vggsound_chem_grid_lrs2_synchformer \
#   --audio_length 10 \
#   --batch_size 16 \
#   --num_workers 16 \
#   --device 0 1 2 3  \
#   --synchformer_ckpt /home/chengxin/chengxin/vasflow/assets/synchformer/synchformer_state_dict.pth &

if __name__ == '__main__':
    import argparse
    
    parser = argparse.ArgumentParser(description='Extract synchformer features from videos')
    parser.add_argument('--video_paths', type=str, nargs='+', required=True, help='Path to directory containing video files')
    ...
    parser.add_argument('--device_ids', type=int, nargs='+', default=[0], help='Device indices, e.g. 0 1 2')
    ...
    for video_path in args.video_paths:
        p = set(glob(f'{video_path}/**/*.mp4',recursive=True) + glob(f'{video_path}/*.mp4',recursive=True)) 
        p = list(p)
        log.info(f"{video_path}, {len(p)}")
        video_paths += p
    video_paths = [Path(f) for f in video_paths]
    log.info(f"A total of {len(video_paths)} videos")
    log.info(f"Devices: {args.device_ids}")
    ...
    devices = [f'cuda:{idx}' for idx in args.device_ids]
    chunks = split_into_chunks(video_paths, len(devices))
    ...
    for device, chunk in zip(devices, chunks):
        ...
        proc = mp.Process(
            ...
            name=f'extract-{device}',
        )
        proc.start()
        procs.append(proc)
        log.info('Started %s with %d videos', proc.name, len(chunk))

    errors = []
    for proc in procs:
        proc.join()
        if proc.exitcode not in (0, None):
            errors.append((proc.name, proc.exitcode))

    if errors:
        raise RuntimeError('Some workers failed: ' + ', '.join(f'{name} (code {code})' for name, code in errors))
