...
def main():
    ...
    parser.add_argument('--lrs3', type=str, default='/home/chengxin/chengxin/vssflow/feature/avhubert/infer_data2', help='lrs3 root dir')
    parser.add_argument('--valid-ids', type=str, help='a list of valid ids')
    parser.add_argument('--vocab-size', type=int, default=4396, help='a list of valid ids')
    args = parser.parse_args()
    file_list, label_list = f"{args.lrs3}/files/file.list", f"{args.lrs3}/files/label.list"
    ...
    nframes_audio_file, nframes_video_file = f"{args.lrs3}/avhubert_features/nframes.audio.0", f"{args.lrs3}/avhubert_features/nframes.video.0"
    ...
    temp_text_path = 'temp_text'
    with open(temp_text_path, "w", encoding="utf-8") as f:
        for line in open(label_list, encoding="utf-8"):
            f.write(line.strip().lower() + "\n")
    gen_vocab(Path(temp_text_path), vocab_dir / spm_filename_prefix, 'unigram', args.vocab_size)
    ...
    audio_dir = '/home/chengxin/chengxin/dataset/GRID/preprocessed_data/speakers'
    video_dir = '/home/chengxin/chengxin/dataset/GRID/preprocessed_data/videos_25fps'

    def setup_target(target_dir, train):
        for name, data in zip(['all_data'], [train]):
    ...
    for fid, label, nf_audio, nf_video in zip(fids, labels, nfs_audio, nfs_video):
        all_data.append([fid, label, nf_audio, nf_video])
        # part = fid.split('/')[0]
        # # print(part)
        # if part == 'test':
    ...
    dir_433h = f"{args.lrs3}/all_data"
    print(f"Set up 433h dir")
    os.makedirs(dir_433h, exist_ok=True)
    print(dir_433h, vocab_path)
    setup_target(dir_433h, all_data)
    return
